...
class Autocomplete extends React.Component {
  ...
  static defaultProps = {
    ...
    renderMenu(items, value, style, suggestionsMenuId) {
      return <div id={suggestionsMenuId} role="listbox" style={{ ...style, ...this.menuStyle }}>{items}</div>
    },
    menuStyle: {
      borderRadius: '3px',
  ...
  UNSAFE_componentWillReceiveProps(nextProps) {
    ...
    // @TODO: Find way to filter elements onChange rather on relying on componentWillReceiveProps
  ...
  exposeAPI = () => {
    ...
    // IMPERATIVE_API.forEach(event => {
    //     const exists = this.references.input.current && this.references.input.current[event];
    //     return this[event] = exists ? this.references.input.current[event].bind(this.references.input.current): null;
    //   });
  }
  ...
  renderMenu = () => {
    ...
    const items = this.getFilteredItems(this.props).map((item, index) => {
      const itemRendered = this.props.renderItem(
        item,
    ...
    const menu = this.props.renderMenu(items, this.props.value, style, this.props.suggestionsMenuId)
    ...
    return (
      <div style={{ ...this.props.wrapperStyle }} {...this.props.wrapperProps}>
        {this.props.renderInput({
          ...inputProps,
          'aria-owns': this.props.suggestionsMenuId,
          'aria-expanded': open,
