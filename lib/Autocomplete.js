...
class Autocomplete extends React.Component {
  ...
  getMemoizedFilteredItems = memoize(({ items, value })=> this.getFilteredItemsToMemoize(items, value));
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (this.props.autoHighlight && (prevProps.value !== this.props.value || highlightedIndex === null)) {
  ...
  exposeAPI = () => {
    IMPERATIVE_API.forEach(event => {
      const enventHandler = this.references.input.current && this.references.input.current[event];
      if (enventHandler) {
        this[event] = function() {}
        this[event] = this[event].bind(enventHandler)
      }
      return this[event];
    });
  }
  ...
  maybeScrollItemIntoView = () => {
    ...
    const itemNode = highlightedIndex !== null ? this.references[`item-${highlightedIndex}`].current : null;
    const menuNode = this.references.menu.current;

    if (this.isOpen() && !!itemNode && !!menuNode) {
      scrollIntoView(
        itemNode,
        menuNode, {
  ...
  keyDownHandlers = {
    ...
    ArrowUp(event) {
      ...
      let index = highlightedIndex === null ? memoizedItems.length : highlightedIndex

      for (let i = 0; i < memoizedItems.length; i++) {
        const p = (index - (1 + i) + memoizedItems.length) % memoizedItems.length
        if (isItemSelectable(memoizedItems[p])) {
          index = p
          break
        }
  ...
  renderItem = (memoizedItems) => {
    ...
    const items = memoizedItems.map((item, index) => {
      ...
      return React.cloneElement(itemRendered, {
        onMouseEnter: isItemSelectable(item) ? () => this.highlightItemFromMouse(index) : null,
        onClick: isItemSelectable(item) ? () => this.selectItemFromMouse(item) : null,
  ...
  renderMenu = (memoizedItems) => {
    ...
    const menu = renderMenu(items, suggestionsMenuId, value, {...menuStyle, ...style})
  ...
  composeEventHandlers = (internalEvent, externalEvent) => {
    return externalEvent ? event => { internalEvent(event); externalEvent(event); } : internalEvent
  }
  ...
  render() {
    ...
    const highlightedItemRef = this.references[`item-${highlightedIndex}`] ? this.references[`item-${highlightedIndex}`].current: null;
    const highlightedItemId = highlightedItemRef ? highlightedItemRef.getAttribute('id'): '';
    ...
    return (
      <div style={{ ...wrapperStyle }} {...wrapperProps}>
        <div role="combobox" aria-expanded={open} aria-owns={suggestionsMenuId} aria-haspopup="listbox">
        {renderInput({
          ...inputProps,
        ...
        <span style={srOnly} role="status" aria-live="assertive" aria-atomic="true" aria-relevant="aditions">
        {open &&
          `${memoizedItems.length} ${numberOfResultsAvailableCopy}`
        }
        </span>
        {open && this.renderMenu(memoizedItems)}
        {debug && (
