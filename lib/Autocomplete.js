...
class Autocomplete extends React.Component {
  ...
  getFilteredItems = memoize(({ items, value })=>{ this.getFilteredItemsToMemoize(items, value)});
  ...
  exposeAPI = () => {
    IMPERATIVE_API.forEach(event => {
      return this[event] = this.references.input.current && this.references.input.current[event] && this.references.input.current[event].bind(this.references.input);
    });
  }
  ...
  setMenuPositions = () => {
    const node = this.references.input.current;
  ...
  render() {
    ...
    const filteredItems = this.getFilteredItems({items, value});
    ...
    return (
      <div style={{ ...wrapperStyle }} {...wrapperProps}>
        ...
        {open &&
          <span style={{...srOnly}} role="alert" aria-live="assertive" aria-atomic="true" aria-relevant="text">
          {filteredItems.length} {numberOfResultsAvailableCopy}
          </span>
        }
        {debug && (
