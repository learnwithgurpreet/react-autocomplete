...
describe('Autocomplete acceptance tests', () => {
  ...
  it('should show results when value is a partial match', () => {
    const { getByTestId, getByRole, rerender } = renderElementAsDom({ open: true });
  ...
  it('should preserve the matched item even when its previous place was outside of the new range', () => {
    const { getByRole, getByTestId, rerender } = renderElementAsDom({ open: true, value: '' });
    const input = getByTestId('inputField');
    fireEvent.focus(input);
  ...
  it('should display menu based on `props.open` when provided', () => {
    autocompleteWrapper = mount(AutocompleteComponentJSX({}))
    expect(autocompleteWrapper.state('isOpen')).toBe(false)
    expect(autocompleteWrapper.find('[role="listbox"]').length).toBe(0)
    autocompleteWrapper.setState({ isOpen: true })
    expect(autocompleteWrapper.find('[role="listbox"]').length).toBe(1)
    autocompleteWrapper.setProps({ open: false })
    autocompleteWrapper.setState({ isOpen: false })
    expect(autocompleteWrapper.find('[role="listbox"]').length).toBe(0)
    autocompleteWrapper.setProps({ open: true })
    expect(autocompleteWrapper.find('[role="listbox"]').length).toBe(1)
  })

  it('should set menu positions on initial render if the menu is visible', () => {
    const menuSpy = jest.fn(() => <div role="listbox" />)
    const wrapper = mount(AutocompleteComponentJSX({
      open: true,
      renderMenu: menuSpy,
    }))
    expect(menuSpy).toHaveBeenCalledTimes(2)
    expect(menuSpy.mock.calls[0][2]).toEqual(wrapper.instance().props.menuStyle)
  })
  ...
  it('should set menu positions when the menu is forced open via props.open', () => {
    ...
    expect(menuSpy).toHaveBeenCalledTimes(2)
    expect(menuSpy.mock.calls[0][2]).toEqual(tree.instance().props.menuStyle)
  })
...
describe('focus management', () => {
  ...
  it('should preserve focus when clicking on a menu item', () => {
    ...
    expect(autocompleteWrapper.state('isOpen')).toBe(true)
    const menu = autocompleteWrapper.find('div > div').at(0)
...
describe('Autocomplete mouse event handlers', () => {
  ...
  it('should open menu if it is closed when clicking in the input', () => {
    expect(autocompleteWrapper.state('isOpen')).toBe(false)
    autocompleteWrapper.instance().isInputFocused = jest.fn(() => true)
    autocompleteInputWrapper.simulate('click')
    expect(autocompleteWrapper.state('isOpen')).toBe(true)
  })

  // for some reason it fails, however the state is updated.
  xit('should set `highlightedIndex` when hovering over items in the menu', () => {
    const tree = mount(AutocompleteComponentJSX({ open: true }))
    tree.setState({ isOpen: true });
...
describe('Autocomplete#renderMenu', () => {
  ...
  it('should return the ref to the menu ReactComponent when renderMenu() is called', () => {
    ...
    expect(typeof autocompleteMenu.ref).toEqual('object')
