...
describe('Autocomplete acceptance tests', () => {
  ...
  it('should show results when value is a partial match', () => {
    ...
    expect(autocompleteWrapper.find('[role="listbox"]').children().length).toBe(50)
    autocompleteWrapper.setProps({ value: 'Ar' })
    expect(autocompleteWrapper.find('[role="listbox"]').children().length).toBe(6)
  ...
  it('should display menu based on `props.open` when provided', () => {
    ...
    expect(tree.find('[role="listbox"]').length).toBe(0)
    tree.setState({ isOpen: true })
    expect(tree.find('[role="listbox"]').length).toBe(1)
...
describe('focus management', () => {
  ...
  it('...even if `input.focus()` happens async (IE)', () => {
    ...
    const input = tree.find('input')
    input.simulate('focus')
    ac.references.input.current.focus = jest.fn()
    expect(tree.state('isOpen')).toBe(true)
    const menu = tree.find('div > div').at(0)
    menu.simulate('mouseEnter')
    expect(ac._ignoreBlur).toBe(true)
    ...
    const items = tree.find('div > div > div')
    items.at(3).simulate('click')
    expect(ac.references.input.current.focus).toHaveBeenCalledTimes(1)
  ...
  it('should preserve focus when clicking on non-item elements in the menu', () => {
    ...
    const input = tree.find('input')
    input.simulate('focus')
    ac.references.input.current.focus = jest.fn(() => input.simulate('focus'))
    expect(tree.state('isOpen')).toBe(true)
    const menu = tree.find('div > div').at(0)
    menu.simulate('mouseEnter')
    expect(ac._ignoreBlur).toBe(true)
...
describe('Public imperative API', () => {
  it('should expose select APIs available on HTMLInputElement', () => {
    ...
    const firstNode = tree.find('[role="listbox"]');
    expect(typeof firstNode.focus).toBe('function')
    expect(firstNode.isInputFocused()).toBe(false)
    firstNode.focus()
    expect(firstNode.isInputFocused()).toBe(true)
    expect(typeof firstNode.setSelectionRange).toBe('function')
    ...
    expect(typeof firstNode.blur).toBe('function')
    firstNode.blur()
    expect(firstNode.isInputFocused()).toBe(false)
  })
})
